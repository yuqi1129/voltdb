...
@VOLT.Command(
    ...
    options = (
            VOLT.StringListOption(None, '--ignore', 'skip_requirements',
                                  ...
                                  default = ''),
            VOLT.IntegerOption(None, '--shutdown-delay', 'shutdown_delay', 'Delay the shutdown of the hosts which are being removed. This needs to be specified if topics are being used.', default=-1),
...
def resize(runner):
    ...
    # Convert shutdown delay input of seconds to millis
    shutdown_delay = runner.opts.shutdown_delay * 1000 if runner.opts.shutdown_delay > 0 else -1
